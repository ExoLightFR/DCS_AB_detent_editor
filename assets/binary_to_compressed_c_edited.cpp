...
static bool binary_to_compressed_c(const char* filename, const char* symbol, bool use_base85_encoding, bool use_compression, bool use_global);
...
int main(int argc, char** argv)
{
    ...
    while (argn < (argc - 2) && argv[argn][0] == '-')
    {
        ...
        else if (strcmp(argv[argn], "-noglobal") == 0) { use_global = false; argn++; }
        else
        {
    ...
    bool ret = binary_to_compressed_c(argv[argn], argv[argn + 1], use_base85_encoding, use_compression, use_global);
    if (!ret)
...
bool binary_to_compressed_c(const char* filename, const char* symbol, bool use_base85_encoding, bool use_compression, bool use_global)
{
    // Read file
    ...
    const char* global_str = use_global ? "extern " : "";
    const char* compressed_str = use_compression ? "compressed_" : "";
    if (use_base85_encoding)
    {
        fprintf(out, "%sconst char %s_%sdata_base85[%d+1] =\n    \"", global_str, symbol, compressed_str, (int)((compressed_sz + 3) / 4)*5);
        char prev_c = 0;
    ...
    {
        fprintf(out, "%sconst unsigned int %s_%ssize = %d;\n", global_str, symbol, compressed_str, (int)compressed_sz);
        fprintf(out, "%sconst unsigned int %s_%sdata[%d/4] =\n{", global_str, symbol, compressed_str, (int)((compressed_sz + 3) / 4)*4);
        int column = 0;
